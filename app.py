...
""")
    ...
    with tab_actualizar:
        st.subheader("🔄 Modificación Estructural de Registros")
    
        if not st.session_state.crud_datos.empty:
            id_a_modificar = st.selectbox(
            "Seleccione el código ID que desea editar:", 
            ...
            with col_u1:
                ...
                u_precio = st.number_input("Modificar Precio (S/):", value=float(datos_actuales["Precio"]), min_value=0.0, format="%.2f", key="crud_u_prc")
            with col_u2:
            ...
            if st.button("Confirmar Cambios Operativos", key="btn_crud_update"):
               if u_nombre.strip() == "":
                   ...
                   st.session_state.crud_datos.loc[indice, "Total (S/)"] = objeto_actualizado.calcular_valor_total()
